...
func (h *HeadBanner) List() error {
	...
	if modelId == 0 {
		return h.Json(http.StatusInternalServerError, env.ErrMissReqParam)
	}
...
func (h *HeadBanner) Save() error {
	...
	if data.Id == 0 {
		return h.Json(http.StatusInternalServerError, env.ErrOperationFailed)
	}
